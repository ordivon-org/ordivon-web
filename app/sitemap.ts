...
export default function sitemap(): MetadataRoute.Sitemap {
  ...
  const writingRoutes = articles.map((article) => ({ url: `${base}/writing/${article.slug}`, lastModified: article.modifiedDate || article.date, changeFrequency: "monthly" as const, priority: .7 }));
