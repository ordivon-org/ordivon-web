...
That authority may belong to a human, an Agent, or another admitted workflow depending on the consequence. The ordinary machine entrypoint is `promotion:admit`: its preflight first requires the exact candidate revision to have entered canonical Web source through `origin/main`, then validates the owner read-set, runs the full Web gate, prepares the candidate, and **revalidates the owner read-set at the final admission point** before creating or reconciling the exact source-bound promotion tag. This source-integration gate prevents a published detached revision from leaving the default Agent source behind; it does not make `main` publication authority. `promotion:preflight` remains independently callable for inspection and troubleshooting; it is not an extra ceremony the ordinary caller must sequence before admission.
