...
Current project summaries are public projections at `siteUpdatedAt`. Some remain directly authored; Harness and Security are compiled from source-bound owner snapshots plus Web editorial judgment. Their owner repositories may organize current truth differently—the projection contract follows each authority map rather than requiring a shared STATUS schema. The owning repositories remain authoritative for code, tests, releases, operations, and current technical boundaries. Research records remain authoritative for setup and evidence. Articles remain authoritative for their dated argument. Generated modules, manifests, and pages remain rebuildable projections.
