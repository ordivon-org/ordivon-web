...
- Ordinary Web source commits do **not** imply publication. `pnpm promotion:admit` is the ordinary semantic publication entrypoint and invokes `promotion:preflight` internally. That preflight first requires the exact candidate revision to be integrated into `origin/main`, then revalidates every source-projected owner envelope, runs the full Web verification gate, prepares the Pages artifact, and revalidates the owner read-set again at the publication-admission boundary. `origin/main` is source-integration/currentness evidence, not publication authority; `promotion:preflight` and `pages:prepare` remain independently callable lower-level inspection/troubleshooting primitives, not prerequisite commands an ordinary caller must sequence manually.
...
- `pnpm promotion:publish <tag>` is the separate remote-publication boundary. It requires checkout `HEAD` to equal the admitted tag target, revalidates the checked-out source's owner envelopes against current owners, cross-checks that read-set against the receipt, and permits only an absent-or-identical remote tag ref before an exact non-force push. For Web source-review currentness, the fresh default-branch projection of the already reviewed `project.repository` locator is the canonical publication horizon, while the current local owner-visible public projection is a prospective-pressure witness. A changed remote envelope always blocks publication review. Local admitted public work that is ahead of or divergent from that reviewed horizon also blocks review, while a local public revision proven by full-history ancestry to be strictly behind the unchanged reviewed horizon is recorded as superseded and does not force Web to mutate or fast-forward the owner checkout. Dirty, unadmitted, or ancestry-unknown local pressure remains fail-closed. A local worktree, local `main`, local `origin`, or remote-tracking ref cannot select the remote authority. When local source differs from the freshly observed head, Web materializes that reviewed repository horizon in disposable storage and compares the public envelope with complete branch history, so unrelated remote commits do not force rebind and currentness observation does not mutate the owner repository. This creates Web review obligations only; it does not declare the remote branch to be the owner domain's semantic truth. `--dry-run` performs the same semantic checks without remote mutation. A later owner-envelope change blocks first remote publication and creates a new Web review obligation; an already published historical tag remains a historical fact.
